...
export function FinalRenderControls({
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-950/50 p-6" aria-labelledby="assemble-stage-heading">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Production stage</p>
        <h3 id="assemble-stage-heading" className="mt-2 text-2xl font-semibold text-white">4. Assemble</h3>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Validate final-render prerequisites, queue deterministic assembly, and hand completed PAK-owned output to the operational Media Library.</p>
      </div>
      ...
      <div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/70 p-4">
        ...
        {assembly?.state === "COMPLETED" && assembly.finalMediaAssetId ? (
          <div className="mt-4 rounded-xl border border-emerald-900/60 bg-emerald-950/25 p-4">
            ...
            <p className="mt-2 text-xs text-emerald-200/75">Final asset ID <span className="font-mono">{assembly.finalMediaAssetId}</span> · Assembly completed</p>
            <Link
              href="/media-library"
              className="mt-3 inline-block text-sm font-medium text-emerald-300 underline underline-offset-4"
            >
              Open final asset in Media Library
            </Link>
          </div>
