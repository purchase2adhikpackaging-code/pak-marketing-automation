...
function targetMeta(item: ApprovalQueueItem): string {
  if (item.target.type === "CONTENT_ARTIFACT") return `${item.target.language} · Revision ${item.target.revision}`;
...
function appendUnique(current: ApprovalQueueItem[], incoming: ApprovalQueueItem[]): ApprovalQueueItem[] {
...
export function ApprovalCenterClient({ organizations }: { organizations: ApprovalCenterOrganization[] }) {
  ...
  function loadMore() {
    if (!page.nextCursor || isPending) return;
    ...
    startTransition(async () => {
      try {
        ...
        setPage((current) => ({
          items: appendUnique(current.items, response.page.items),
    ...
    <div className="mt-8 space-y-6">
      <section className="rounded-2xl border border-slate-800 bg-slate-950/60 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <label className="block max-w-xl flex-1 space-y-2">
            ...
            <select value={currentOrganization.id} onChange={(event) => selectOrganization(event.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-sm text-white outline-none focus:border-slate-500" disabled={isPending}>
              {organizations.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.label}</option>)}
            </select>
          </label>
      ...
      <section className="rounded-2xl border border-indigo-900/50 bg-indigo-950/20 p-5">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-300">{currentOrganization.sceneReviewRequired} scene plans require domain review in Scene Planning.</p>
          </div>
          <Link href="/scene-planning" className="inline-flex min-h-10 items-center justify-center rounded-xl border border-indigo-700 px-4 py-2 text-sm font-semibold text-indigo-100 hover:bg-indigo-950/60">Open Scene Planning</Link>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950/60 p-5">
        <div className="flex flex-wrap gap-2" aria-label="Approval workflow status">
          {STATUS_TABS.map((tab) => (
            <button key={tab.status} type="button" onClick={() => loadStatus(tab.status)} disabled={isPending} className={`rounded-xl border px-3 py-2 text-sm font-semibold transition ${activeStatus === tab.status ? "border-slate-500 bg-slate-800 text-white" : "border-slate-800 text-slate-400 hover:border-slate-700 hover:text-slate-200"}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        {isPending && !isLoadingMore ? <p className="mt-6 text-sm text-slate-400">Loading approval requests…</p> : null}

        {!isPending && page.items.length === 0 ? (
          <div className="mt-6 rounded-xl border border-dashed border-slate-800 p-6 text-sm text-slate-500">{activeTab.empty}</div>
        ) : null}
              ...
              <article key={item.id} className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-slate-700 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{item.targetType === "CONTENT_ARTIFACT" ? "Content" : "Media"}</span>
                    <span className="rounded-full border border-slate-700 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{item.status.replaceAll("_", " ")}</span>
                  </div>
                  ...
                  <p className="mt-2 text-xs text-slate-500">Requested {formatTime(item.requestedAt)}{item.requestedBy ? ` · ${item.requestedBy.slice(0, 8)}…` : ""}</p>
                </div>
                <Link href={`/approval-center/${item.id}?organization=${currentOrganization.id}`} className="inline-flex min-h-10 shrink-0 items-center justify-center rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-slate-200">Review request</Link>
              </article>
            ))}
          </div>
        ) : null}
        ...
        {page.nextCursor ? (
          <div className="mt-4 flex justify-center">
            <button type="button" onClick={loadMore} disabled={isPending} className="rounded-xl border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-200 hover:border-slate-500 disabled:opacity-50">
