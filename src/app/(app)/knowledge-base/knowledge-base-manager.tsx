...
export function KnowledgeBaseManager({
  ...
  organizations: KnowledgeOrganizationWorkspace[];
}) {
  ...
  function createRecord() {
    ...
    startTransition(async () => {
      const result = await createKnowledgeAction({
        ...
        ...(createForm.sourceReference.trim()
          ? { sourceReference: createForm.sourceReference.trim() }
          : {}),
      });
  ...
  function saveEdit(record: KnowledgeRecord) {
    ...
    startTransition(async () => {
      const result = await updateKnowledgeAction({
        ...
        ...(editForm.sourceReference.trim()
          ? { sourceReference: editForm.sourceReference.trim() }
          : {}),
      });
    ...
    <div className="mt-8 space-y-6">
      ...
      {canManage ? (
        <section className="rounded-2xl border border-slate-800 bg-slate-950/60 p-6">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setCreateForm((current) => ({ ...current, sourceLabel: event.target.value }))
                }
                className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-sm text-white"
                disabled={isPending}
              />
            ...
            <label className="space-y-2 md:col-span-2">
              <span className="text-sm font-medium text-slate-200">Source reference</span>
              <input
                aria-label="Source reference"
                value={createForm.sourceReference}
                onChange={(event) =>
                  setCreateForm((current) => ({ ...current, sourceReference: event.target.value }))
                }
                className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-sm text-white"
                disabled={isPending}
              />
          ...
          <button
            ...
            disabled={isPending || createForm.title.trim().length < 3 || createForm.content.trim().length === 0}
            className="mt-5 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-950 disabled:opacity-50"
          >
            Create draft
          </button>
        </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-white">Knowledge records</h3>
            <p className="mt-1 text-sm text-slate-500">{visibleRecords.length} record{visibleRecords.length === 1 ? "" : "s"}</p>
          </div>
        </div>

        {visibleRecords.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-800 p-6 text-sm text-slate-500">
            No Knowledge Base records are available for this organization.
          </div>
        ) : null}
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {canDelete ? (
                      <button
                        ...
                        disabled={isPending}
                        className="rounded-lg border border-red-900/70 px-3 py-2 text-xs font-semibold text-red-200"
                      >
                        Delete
                      </button>
                    ) : null}
              ...
              {editing ? (
                <div className="mt-5 space-y-4 border-t border-slate-800 pt-5">
                  <label className="block space-y-2">
                    <span className="text-sm text-slate-300">Edit title</span>
                    <input
                  ...
                  <label className="block space-y-2">
                    <span className="text-sm text-slate-300">Edit content</span>
                    <textarea
                ...
                <div className="mt-5 space-y-3 text-sm leading-6 text-slate-300">
                  ...
                  <div className="text-xs text-slate-500">
                    ...
                    {record.sourceLabel ? <span className="ml-3">{record.sourceLabel}</span> : null}
                    {record.sourceReference ? <span className="ml-3">{record.sourceReference}</span> : null}
                  </div>
                </div>
              )}
            </article>
