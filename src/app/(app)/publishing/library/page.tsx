      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        {rows.length === 0 ? (
          ...
          <table className="min-w-full text-left text-sm text-black">
            <thead className="text-black"><tr><th className="py-2">Programme</th><th>Subject</th><th>Edition</th><th>Revision</th><th>Released</th><th>PDF</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {rows.map(({ publication, url }) => (
                <tr key={publication.id}>
                  <td className="py-3 font-medium text-black">{publication.programmeCode}</td>
                  ...
                  <td>{url ? <a href={url} target="_blank" rel="noreferrer" className="text-black underline">Open PDF</a> : "Unavailable"}</td>
                </tr>
              ))}
            </tbody>
          </table>
