...
function actionForTarget(artifact: ScriptArtifact | undefined): {
  label: string;
  ariaLabel: string;
  disabled: boolean;
} {
  if (!artifact || artifact.status === "PENDING") {
    return { label: "Generate translation", ariaLabel: "Generate", disabled: false };
  }

  if (artifact.status === "GENERATING") {
    return { label: "Generating…", ariaLabel: "generating", disabled: true };
  }

  if (artifact.status === "FAILED") {
    return { label: "Retry translation", ariaLabel: "Retry", disabled: false };
  }
...
export function MultilingualContentPanel({
  ...
  const byLanguage = useMemo(() => {
    ...
    for (const artifact of artifacts) {
      map.set(artifact.language, artifact);
    }
    return map;
  }, [artifacts]);
  ...
  function replaceArtifact(next: ScriptArtifact) {
    setArtifacts((current) => {
      const withoutLanguage = current.filter((artifact) => artifact.language !== next.language);
      return [...withoutLanguage, next];
    });
  }
      ...
      setArtifacts((current) =>
        current.map((artifact) => {
          ...
          if (
            ...
            (artifact.sourceRevision ?? 0) < response.artifact.revision
          ) {
  ...
  return (
    <section className="mt-8" aria-label="Multilingual scripts">
      ...
      {error ? (
        <div className="mt-5 rounded-xl border border-red-900/60 bg-red-950/30 p-4 text-sm leading-6 text-red-200" role="alert">
          {error}
        </div>
      ) : null}
        ...
        {LANGUAGES.map(({ code, label }) => {
          ...
          return (
            <article
              ...
              className="flex min-h-[24rem] flex-col rounded-2xl border border-slate-800 bg-slate-950/60 p-5 shadow-sm"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-2">
                  {source ? (
                    <span className="rounded-full border border-emerald-800 bg-emerald-950/40 px-2.5 py-1 text-xs font-medium text-emerald-200">
                      Canonical source
                    </span>
                  ) : null}
              ...
              <div className="mt-4 flex-1 whitespace-pre-wrap text-sm leading-7 text-slate-200">
                {artifact?.scriptText?.trim() || "No script exists for this language yet."}
              </div>
              ...
              <div className="mt-5 border-t border-slate-800 pt-4">
                {source ? (
                  <button
                    type="button"
                    onClick={() => regenerateSource(artifact)}
                    disabled={busy || artifact.status === "GENERATING"}
                    aria-label={`Regenerate ${label} source`}
