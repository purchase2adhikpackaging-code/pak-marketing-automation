...
export function BrandKitClient({ organizations }: { organizations: BrandKitWorkspace[] }) {
  ...
  const setField = (field: keyof Omit<BrandForm, "approvedImageryAssetIds">, value: string) => setForm((current) => ({ ...current, [field]: value }));
  ...
  const save = () => {
    ...
    startTransition(async () => {
      ...
      setWorkspaces((current) => current.map((workspace) => workspace.id === selected.id ? { ...workspace, brandKit: result.brandKit } : workspace));
  ...
  const renderAssetSelect = (label: string, field: keyof Pick<BrandForm, "primaryLogoAssetId" | "lightLogoAssetId" | "darkLogoAssetId" | "brandMarkAssetId" | "faviconAssetId">) => (
    <label className={labelClass}>
      ...
      <select aria-label={label} className={inputClass} disabled={!canEdit} value={form[field]} onChange={(event) => setField(field, event.target.value)}>
        ...
        {selected.imageAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.displayName}</option>)}
      </select>
    </label>
  );
  ...
  return (
    <div className="mt-7 space-y-6">
      ...
      {!canEdit ? <p className="rounded-lg border border-slate-800 bg-slate-900/50 px-4 py-3 text-sm text-slate-300">Owner or Admin access is required to edit Brand Kit. You can review the current authoritative brand settings.</p> : null}
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {renderAssetSelect("Primary logo", "primaryLogoAssetId")}{renderAssetSelect("Light logo", "lightLogoAssetId")}{renderAssetSelect("Dark logo", "darkLogoAssetId")}{renderAssetSelect("Brand mark", "brandMarkAssetId")}{renderAssetSelect("Favicon", "faviconAssetId")}
      </div>
      <div className="grid gap-5 md:grid-cols-2">
      ...
      <fieldset className="rounded-xl border border-slate-800 p-4" disabled={!canEdit}>
        ...
        {selected.imageAssets.length === 0 ? <p className="text-sm text-slate-400">Upload approved images to Media Library first.</p> : <div className="grid gap-2 md:grid-cols-2">{selected.imageAssets.map((asset) => <label key={asset.id} className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={form.approvedImageryAssetIds.includes(asset.id)} onChange={() => toggleApprovedImagery(asset.id)} />{asset.displayName}</label>)}</div>}
      </fieldset>
      <div className="flex items-center gap-4">
        {canEdit ? <button type="button" disabled={isPending} onClick={save} className="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-950 disabled:opacity-60">{isPending ? "Saving…" : "Save Brand Kit"}</button> : null}
