...
describe("approved Scene Planning lifecycle SQL", () => {
  it("allows only lifecycle-only APPROVED -> STALE/SUPERSEDED transitions", () => {
    ...
    expect(sql).toContain("new.canonical_narration is not distinct from old.canonical_narration");
    expect(sql).toContain("new.visual_bible_snapshot is not distinct from old.visual_bible_snapshot");
    expect(sql).toContain("new.qc_summary is not distinct from old.qc_summary");
  });
