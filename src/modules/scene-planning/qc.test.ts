...
import { runScenePlanQc, type ScenePlanQcInput } from "./qc";
        ...
        shots: [
          {
            ...
            narrationEndChar: 12,
            narrationText: canonicalNarration.slice(0, 12),
            creativeDirection: "Trainer enters the railway systems laboratory.",
            masterVisualPrompt: "Photorealistic European railway training laboratory, trainer entering frame.",
            aspectRatio: "16:9",
        ...
        shots: [
          {
            ordinal: 1,
            durationSeconds: 5,
            narrationStartChar: 12,
            narrationEndChar: canonicalNarration.length,
            narrationText: canonicalNarration.slice(12),
            creativeDirection: "Trainees inspect a railway control panel with the trainer.",
            masterVisualPrompt: "Photorealistic trainees inspecting a railway control panel under practical lighting.",
            aspectRatio: "16:9",
            subjectRefs: ["person:trainer-1"],
            locationRefs: ["location:lab-1"],
            cameraMotion: "controlled lateral track",
            generationRequirements: { preserveNarration: true, providerNeutral: true },
          },
        ],
      },
...
describe("runScenePlanQc", () => {
  ...
  it("treats whitespace-only gaps between exact narration spans as fully covered", () => {
    ...
    input.scenes[0].shots[0].narrationEndChar = 11;
    input.scenes[0].shots[0].narrationText = canonicalNarration.slice(0, 11);
    input.scenes[1].shots[0].narrationStartChar = 12;
    input.scenes[1].shots[0].narrationText = canonicalNarration.slice(12);
  ...
  it("blocks stale source, narration gaps, overlap, and rewritten narration", () => {
    ...
    gap.scenes[1].shots[0].narrationStartChar = 14;
    gap.scenes[1].shots[0].narrationText = canonicalNarration.slice(14);
    expect(codes(runScenePlanQc(gap), "BLOCKER")).toContain("NARRATION_GAP");

    const overlap = validInput();
    overlap.scenes[1].shots[0].narrationStartChar = 10;
    overlap.scenes[1].shots[0].narrationText = canonicalNarration.slice(10);
    expect(codes(runScenePlanQc(overlap), "BLOCKER")).toContain("NARRATION_OVERLAP");

    const rewritten = validInput();
    rewritten.scenes[0].shots[0].narrationText = "Rewritten narration";
  ...
  it("blocks scene-duration mismatch and warns for camera conflict, duplicate ideas, and target-duration drift", () => {
    ...
    sceneDuration.scenes[0].durationSeconds = 7;
    expect(codes(runScenePlanQc(sceneDuration), "BLOCKER")).toContain("SCENE_DURATION_MISMATCH");

    const camera = validInput();
    camera.scenes[0].shots[0].cameraMotion = "locked static tripod with fast orbit around subject";
    expect(codes(runScenePlanQc(camera), "WARNING")).toContain("CAMERA_MOTION_CONFLICT");

    const duplicate = validInput();
    duplicate.scenes[1].shots[0].creativeDirection = duplicate.scenes[0].shots[0].creativeDirection;
