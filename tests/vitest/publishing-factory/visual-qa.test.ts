  ...
  requirements: [
    {
      ...
      subjectPrompt: "front",
      caption: "Front cover",
      altText: "Realistic railway front cover",
      realistic: true,
...
function htmlFor(visuals: ResolvedBookVisualBundle): string {
  return `<main>${visuals.visuals
    .map((visual) => `<figure data-visual-id="${visual.id}"><img src="${visual.dataUri}" alt="${visual.altText}"></figure>`)
    .join("")}</main>`;
}

describe("visual QA", () => {
  it("passes a complete benchmark-compliant visual bundle", () => {
    const visuals = bundle();
    expect(runVisualQa({ plan, bundle: visuals, html: htmlFor(visuals) })).toEqual([]);
  });
  ...
  ])("emits blocking %s finding", (code, mutate) => {
    const findings = runVisualQa({ plan, bundle: mutate(bundle()), html: htmlFor(mutate(bundle())) });
    expect(findings.some((finding) => finding.defectClass === code && finding.severity === "error")).toBe(true);
  });

  it("fails when a planned visual is missing from serialized HTML", () => {
    const visuals = bundle();
    const html = htmlFor(visuals).replace('data-visual-id="technical"', 'data-visual-id="missing"');
    const findings = runVisualQa({ plan, bundle: visuals, html });
    expect(findings.some((finding) => finding.defectClass === "VISUAL_HTML_MISSING")).toBe(true);
  });
});
