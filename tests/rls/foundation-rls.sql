...
begin
  ...
  if insert_policy.policyname is null or insert_policy.cmd <> 'INSERT'
     or insert_policy.with_check not ilike '%has_org_role%organization_id%OWNER%ADMIN%EDITOR%' then
  ...
  if update_policy.policyname is null or update_policy.cmd <> 'UPDATE'
     or update_policy.qual not ilike '%has_org_role%organization_id%OWNER%ADMIN%EDITOR%' then
...
declare
  ...
  snapshot_insert pg_policies%rowtype;
...
begin
  ...
  if knowledge_insert.policyname is null or knowledge_insert.cmd <> 'INSERT'
     or knowledge_insert.with_check not ilike '%has_org_role%organization_id%OWNER%ADMIN%EDITOR%' then
  ...
  if knowledge_update.policyname is null or knowledge_update.cmd <> 'UPDATE'
     or knowledge_update.qual not ilike '%has_org_role%organization_id%OWNER%ADMIN%EDITOR%' then
  ...
  select * into snapshot_insert from pg_policies
  where schemaname = 'public' and tablename = 'content_item_knowledge_sources'
    and policyname = 'content_item_knowledge_sources_insert_editor';
  if snapshot_insert.policyname is null or snapshot_insert.cmd <> 'INSERT'
     or snapshot_insert.with_check not ilike '%has_org_role%organization_id%OWNER%ADMIN%EDITOR%' then
    raise exception 'knowledge snapshot INSERT policy is missing or malformed';
  end if;
  ...
  if exists (
    ...
    raise exception 'knowledge snapshots must not expose authenticated UPDATE or DELETE policies';
  end if;
end
$$;
